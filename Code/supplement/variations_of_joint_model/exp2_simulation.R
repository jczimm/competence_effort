...
joint_w_gini <- NULL
for (i in 1:15) {
  mdl <- paste0(mdl1, as.character(strong[i]), ",", as.character(weak[i]), mdl2, regular, with_gini, mdl3)
  a <- webppl(mdl)
  incentive <- which(a==T)[1]-1
  joint_w_gini <- c(joint_w_gini, incentive)
}

joint_wo_gini <- NULL
for (i in 1:15) {
  mdl <- paste0(mdl1, as.character(strong[i]), ",", as.character(weak[i]), mdl2, regular, without_gini, mdl3)
  a <- webppl(mdl)
  incentive <- which(a==T)[1]-1
  joint_wo_gini <- c(joint_wo_gini, incentive)
}

safe_joint_w_gini <- NULL
for (i in 1:15) {
  mdl <- paste0(mdl1, as.character(strong[i]), ",", as.character(weak[i]), mdl2, safe, with_gini, mdl3)
  a <- webppl(mdl)
  incentive <- which(a==T)[1]-1
  safe_joint_w_gini <- c(safe_joint_w_gini, incentive)
}

incentive <- c(joint_w_gini, joint_wo_gini, safe_joint_w_gini)
incentive[is.na(incentive)==T] <- 0

df <- data.frame(model=c(rep('joint_w_gini',15),rep('joint_wo_gini',15),rep('safe_joint_w_gini',15)))
df$model <- factor(df$model,levels = c('joint_w_gini','joint_wo_gini','safe_joint_w_gini'), ordered = T)
# stronger contestant's strength
df$strong_strength <- rep(c(9.5, 9.5, 9.5, 8.0, 8.0, 8.0, 7.5, 7.5, 7.5, 6.5, 6.5, 6.5, 6.0, 6.0, 6.0), 3)
# weaker contestant's strength
df$weak_strength <- rep(c(9.5, 6.5, 5.0, 8.0, 6.5, 5.0, 7.5, 6.0, 5.0, 6.5, 5.5, 5.0, 6.0, 5.5, 5.0), 3)
# stronger contestant's minimum incentive to lift the box alone
df$strong_incentive <- rep(c(65, 65, 65, 75, 75, 75, 85, 85, 85, 95, 95, 95, 105, 105, 105), 3)
# weaker contestant's minimum incentive to lift the box alone
df$weak_incentive <- rep(c(65, 95, 125, 75, 95, 125, 85, 105, 125, 95, 115, 125, 105, 115, 125), 3)
