...
inference <- read.csv('exp1_simulation.csv', header = T, stringsAsFactors = T)
...
pdf('figS1a.pdf', onefile=T, width = 4, height = 8)

dat$model <- 'data'

figS1a <- plt(dat, inference, 'round', 'strength', pt_size = 1) +
  coord_cartesian(ylim = c(1, 10)) +
  facet_grid(scenario ~ agent)

figS1a + labs(x = 'Round', y='Strength')

dev.off()

pdf('figS1b.pdf', onefile=T, width = 4, height = 5)
...
figS1b <- plt(dat_subset, inference_subset, 'round', 'effort') +
  coord_cartesian(ylim = c(0, 100)) +
  facet_grid(scenario ~ agent)

figS1b + labs(x = 'Round', y='Effort (%)')

dev.off()

pdf('figS1c.pdf', onefile=T, width = 4, height = 3)

inference_subset <- inference %>% filter(round == 3 & agent == 'A')
dat_subset <- dat %>% filter(round == 3 & agent == 'A')

figS1c <- plt(dat_subset, inference_subset, 'scenario', 'prob') +
  coord_cartesian(ylim = c(0, 100)) 

figS1c + labs(x='Round 1 and Round 2 outcome', y='Lift probability in Round 3 (%)', color=NULL)
...
pdf('figS2.pdf', onefile=T, width = 8, height = 7)

figS2L <- cor_plt(inference, 'prob', 'avg_prob', 'lci_prob', 'uci_prob', prob.text.labels) +
  scale_x_continuous(limits = c(0, 100), breaks = c(0,25,50,75,100)) +
  scale_y_continuous(limits = c(0,100), breaks = c(0,25,50,75,100)) +
  coord_fixed()

figS2M <- cor_plt(inference, 'effort', 'avg_effort', 'lci_effort', 'uci_effort', effort.text.labels) +
  scale_x_continuous(limits = c(0, 100), breaks = c(0,25,50,75,100)) +
  scale_y_continuous(limits = c(0,100), breaks = c(0,25,50,75,100)) +
  coord_fixed()

figS2R <- cor_plt(inference, 'strength', 'avg_strength', 'lci_strength', 'uci_strength', strength.text.labels) +
  scale_x_continuous(limits = c(1, 10), breaks = c(1,2.5,5,7.5,10)) +
  scale_y_continuous(limits = c(1,10), breaks = c(1,2.5,5,7.5,10)) +
  coord_fixed()

figS2L + labs(x = NULL, y='Data', title='Probability (%)') + 
  theme(legend.position='none') +
  figS2M + labs(x = 'Model', y=NULL, title='Effort (%)') + 
  theme(legend.position='none') +
  figS2R + labs(x = NULL, y=NULL, title='Strength')
...
# load simulation
df <- read.csv('exp2_simulation.csv', header = T, stringsAsFactors = T)
...
pdf('figS3a.pdf', onefile=T, width = 8, height = 4)

dat2$model <- 'data'

figS3a <- plt(dat2, df, 'weak_strength', 'incentive') +
  coord_cartesian(ylim = c(0, 115)) +
  facet_grid(~strong_strength, scales = 'free')

figS3a + labs(x='Weak contestant\'s strength', y='Incentive', color=NULL)
...
pdf('figS3b.pdf', onefile=T, width = 10, height = 3)

figS3b <- cor_plt2(df2, 'incentive', 'avg_incentive', 'lci_incentive', 'uci_incentive')

figS3b + labs(x = 'Model', y='Data', title='Incentive')
...
t$model <- factor(t$model, levels = c('joint_w_gini','joint_wo_gini','safe_joint_w_gini'), ordered = T)

pdf('figS4b.pdf', onefile=T, width = 7, height = 4)

m$model <- 'data'

figS4b <- plt2(m, t, 'contestant', 'Freq') +
  geom_errorbar(data = m, aes(ymin=p-ci, ymax=p+ci), width=.1,
                position=position_dodge(.9)) +
  scale_y_continuous(limits = c(0,1)) +
  facet_grid(~weight)

figS4b + labs(x='Contestant', y='Proportion')

dev.off()

pdf('figS4c.pdf', onefile=T, width = 5, height = 4)

dat3$model <- 'data'
dat3_subset <- dat3 %>% filter(contestant == 'A')
t_subset <- t %>% filter(contestant == 'A')

figS4c <- plt(dat3_subset, t_subset, 'weight', 'total') +
  coord_cartesian(ylim = c(0, 50))

figS4c + labs(x='Box weight', y='Total incentive allocated')

dev.off()

pdf('figS4d.pdf', onefile=T, width = 7, height = 4)

figS4d <- plt(dat3, t, 'weight', 'incentive') +
  coord_cartesian(ylim = c(0, 50)) +
  facet_grid(~contestant)

figS4d + labs(x='Box weight', y='Incentive')

dev.off()

# pearson correlation
dat_summary <- dat3 %>%
  group_by(weight, contestant) %>%
  dplyr::summarize(avg_incentive = mean(incentive), 
                   uci_incentive = CI(incentive)[1], 
                   lci_incentive = CI(incentive)[3])
t2 <- merge(t, dat_summary, sort = F)

incentive_cor <- NULL
for (i in t2$model %>% unique()) {
  incentive_cor <- c(incentive_cor, cor(t2$avg_incentive[t2$model==i], t2$incentive[t2$model==i], method = 'pearson', use = 'complete.obs'))
}
incentive_cor <- paste0('r = ',round(incentive_cor,2))
incentive.text.labels <- data.frame(model = t2$model %>% unique(),
                                    correlation = incentive_cor,
                                    x = rep(33,3),
                                    y =rep(2.5,3))

pdf('figS5.pdf', onefile=T, width = 10, height = 3)

figS5 <- cor_plt3(t2, 'incentive', 'avg_incentive', 'lci_incentive', 'uci_incentive', 'contestant')

figS5 + labs(x = 'Model', y='Data', title='Incentive', color='Contestant')
