...
pdf('figS6a.pdf', onefile=T, width = 4, height = 8)

dat$model <- 'data'

figS6a <- plt(dat, inference, 'round', 'strength', pt_size = 1) +
  coord_cartesian(ylim = c(1, 10)) +
  facet_grid(scenario ~ agent)

figS6a + labs(x = 'Round', y='Strength')

dev.off()

pdf('figS6b.pdf', onefile=T, width = 4, height = 5)
...
figS6b <- plt(dat_subset, inference_subset, 'round', 'effort') +
  coord_cartesian(ylim = c(0, 100)) +
  facet_grid(scenario ~ agent)

figS6b + labs(x = 'Round', y='Effort (%)')

dev.off()

pdf('figS6c.pdf', onefile=T, width = 4, height = 3)

inference_subset <- inference %>% filter(round == 3 & agent == 'A')
dat_subset <- dat %>% filter(round == 3 & agent == 'A')

figS6c <- plt(dat_subset, inference_subset, 'scenario', 'prob') +
  coord_cartesian(ylim = c(0, 100)) 

figS6c + labs(x='Round 1 and Round 2 outcome', y='Lift probability in Round 3 (%)', color=NULL)
...
pdf('figS7.pdf', onefile=T, width = 8, height = 8)

figS7L <- cor_plt(inference, 'prob', 'avg_prob', 'lci_prob', 'uci_prob', prob.text.labels) +
  scale_x_continuous(limits = c(0, 100), breaks = c(0,25,50,75,100)) +
  scale_y_continuous(limits = c(0,100), breaks = c(0,25,50,75,100)) +
  coord_fixed()

figS7M <- cor_plt(inference, 'effort', 'avg_effort', 'lci_effort', 'uci_effort', effort.text.labels) +
  scale_x_continuous(limits = c(0, 100), breaks = c(0,25,50,75,100)) +
  scale_y_continuous(limits = c(0,100), breaks = c(0,25,50,75,100)) +
  coord_fixed()

figS7R <- cor_plt(inference, 'strength', 'avg_strength', 'lci_strength', 'uci_strength', strength.text.labels) +
  scale_x_continuous(limits = c(1, 10), breaks = c(1,2.5,5,7.5,10)) +
  scale_y_continuous(limits = c(1,10), breaks = c(1,2.5,5,7.5,10)) +
  coord_fixed()

figS7L + labs(x = NULL, y='Data', title='Probability (%)') + 
  theme(legend.position='none') +
  figS7M + labs(x = 'Model', y=NULL, title='Effort (%)') + 
  theme(legend.position='none') +
  figS7R + labs(x = NULL, y=NULL, title='Strength')
...
pdf('figS8a.pdf', onefile=T, width = 8, height = 4)

dat2$model <- 'data'

figS8a <- plt(dat2, df, 'weak_strength', 'incentive') +
  coord_cartesian(ylim = c(0, 115)) +
  facet_grid(~strong_strength, scales = 'free')

figS8a + labs(x='Weak contestant\'s strength', y='Incentive', color=NULL)
...
pdf('figS8b.pdf', onefile=T, width = 10, height = 3)

figS8b <- cor_plt2(df2, 'incentive', 'avg_incentive', 'lci_incentive', 'uci_incentive')

figS8b + labs(x = 'Model', y='Data', title='Incentive')
...
pdf('figS9b.pdf', onefile=T, width = 7, height = 4)

m$model <- 'data'

figS9b <- plt2(m, t, 'contestant', 'Freq') +
  geom_errorbar(data = m, aes(ymin=p-ci, ymax=p+ci), width=.1,
                position=position_dodge(.9)) +
  scale_y_continuous(limits = c(0,1)) +
  facet_grid(~weight)

figS9b + labs(x='Contestant', y='Proportion')

dev.off()

pdf('figS9c.pdf', onefile=T, width = 5, height = 4)

dat3$model <- 'data'
dat3_subset <- dat3 %>% filter(contestant == 'A')
t_subset <- t %>% filter(contestant == 'A')

figS9c <- plt(dat3_subset, t_subset, 'weight', 'total') +
  coord_cartesian(ylim = c(0, 50))

figS9c + labs(x='Box weight', y='Total incentive allocated')

dev.off()

pdf('figS9d.pdf', onefile=T, width = 7, height = 4)

figS9d <- plt(dat3, t, 'weight', 'incentive') +
  coord_cartesian(ylim = c(0, 50)) +
  facet_grid(~contestant)

figS9d + labs(x='Box weight', y='Incentive')
...
pdf('figS10.pdf', onefile=T, width = 10, height = 3)

figS10 <- cor_plt3(t2, 'incentive', 'avg_incentive', 'lci_incentive', 'uci_incentive', 'contestant')

figS10 + labs(x = 'Model', y='Data', title='Incentive', color='Contestant')
