...
for (m in 1:2) {
  if (m == 1) {
    effort_space <- "var efforts = [0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1]"
  }
  else {
    effort_space <- "var efforts = [1]"
  }
  incentive_all <- NULL
  for (c in 1:3) {
    ...
    for (j in 1:3) {
      if (j == 1) {
        mdl <- paste0(effort_space, mdl1, as.character(box_weights[c]), mdl2, as.character(0), mdl3, as.character(1), mdl4)
      } else if (j == 2) {
        mdl <- paste0(effort_space, mdl1, as.character(box_weights[c]), mdl2, as.character(0), mdl3, as.character(2), mdl4)
      } else {
        mdl <- paste0(effort_space, mdl1, as.character(box_weights[c]), mdl2, as.character(1), mdl3, as.character(2), mdl4)
      }
      a <- webppl(mdl)
      df <- bind_rows(a)
      if (j == 1) {
  ...
  if (m == 1) {
    joint_incentive <- incentive_all %>% mutate(model = 'joint')
  }
  else {
    maximum_incentive <- incentive_all %>% mutate(model = 'maximum')
  }
}
...
var jointUtility = function(contestant1,contestant2,contestant1_reward,contestant2_reward){
  ...
  var a = function() {
    ...
    var optEffort = function(strength,strength2,box) {
      return argMax(
        function(effort) {
          return contestant1_reward*lift2(strength,strength2,box,effort,effort2) - a_alpha*effort
        },
        efforts);
    };
    return optEffort(a_strength,b_strength,'box')
  ...
  var b = function() {
    ...
    var optEffort = function(strength,strength2,box) {
      return argMax(
        function(effort) {
          return contestant2_reward*lift2(strength,strength2,box,effort,effort2) - b_alpha*effort
        },
        efforts);
    };
    return optEffort(b_strength,a_strength,'box')
  ...
  var aU = contestant1_reward*lift2(a_strength,b_strength,'box',aE,bE) - a_alpha*aE
  var bU = contestant2_reward*lift2(b_strength,a_strength,'box',bE,aE) - b_alpha*bE
  var table = { aU: aU, bU: bU, aE: aE, bE: bE, jointU: aU+bU, outcome: outcome2(a_strength,b_strength,'box'), a_strength: a_strength, b_strength: b_strength, a_alpha: a_alpha, b_alpha: b_alpha, contestant1: contestant1, contestant2: contestant2, contestant1_reward: contestant1_reward, contestant2_reward: contestant2_reward};
  return table
}
...
for (m in 1:2) {
  if (m == 1) {
    others_effort <- "var others_effort = 0"
  }
  else {
    others_effort <- "var others_effort = 1"
  }
  incentive_all <- NULL
  for (c in 1:3) {
    incentive_contest <- NULL
    for (j in 1:3) {
      if (j == 1) {
        mdl <- paste0(others_effort, mdl1, as.character(box_weights[c]), mdl2, as.character(0), mdl3, as.character(1), mdl4)
      } else if (j == 2) {
        mdl <- paste0(others_effort, mdl1, as.character(box_weights[c]), mdl2, as.character(0), mdl3, as.character(2), mdl4)
      } else {
        mdl <- paste0(others_effort, mdl1, as.character(box_weights[c]), mdl2, as.character(1), mdl3, as.character(2), mdl4)
      }
      a <- webppl(mdl)
      df <- bind_rows(a)
      if (j == 1) {
  ...
  if (m == 1) {
    solitary_incentive <- incentive_all %>% mutate(model = 'solitary')
  }
  else {
    compensatory_incentive <- incentive_all %>% mutate(model = 'compensatory')
  }
}

df <- rbind(joint_incentive, solitary_incentive, compensatory_incentive, maximum_incentive)
